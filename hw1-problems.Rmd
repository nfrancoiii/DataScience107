...
# Put your code here.
# GDP per Capita per Day for Member Groups
years <- seq(1965, 2005, by = 10)
p <- master %>%
  filter(year >= 1950, continent != "Oceania") %>%
  ggplot(aes(x = GDPperCapitaPerDay, weight = pop, frame = year)) +
  geom_density() +
  facet_grid(~year) +
  ggtitle("GDP per Capita per Day in 1970 (Density)") +
  labs(x = "GDP per Capita per Day ($)", y = "Population Weighted Density")
